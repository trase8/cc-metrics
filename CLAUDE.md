...
Ориентиры для Coolify: healthcheck — `GET /health`, HTTPS терминирует Traefik самого Coolify,
`CC_METRICS_TOKEN` задаётся в UI как переменная окружения.

По умолчанию контейнер слушает 4318, но порт берётся из `PORT`, если она задана. Поэтому подходит любой
из двух вариантов: выставить в Coolify «Ports Exposes» = 4318, либо оставить платформе её порт и передать
`PORT=3000`. Несовпадение здесь даёт 502 от Traefik на всех путях — это первое, что стоит проверять,
если домен отвечает, а сервис нет.
